    ...
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            ...
            GenBtn.Click += async (s, e) =>
            {
                ...
                var pngs = imageFiles.Where(filename => filename.EndsWith("png")).Select(filename => NetVips.Image.NewFromFile(filename)).ToArray();
                var framesCount = 0;
                ...
                foreach(var filename in gifFiles)
                {
                    ...
                    {
                        var frame = gif.Crop(0, i * gif.PageHeight, gif.Width, gif.PageHeight);
                        frames.Add(frame);
                    }
                    if (framesCount == 0)
                    {
                ...
                for(var i =0; i < framesCount; i++ )
                {
                    ...
                    foreach (var gif in gifs)
                    {
                        ...
                        blendModes.Add(Enums.BlendMode.Add);
                    }

                    var blendedFrame = baseImage.Composite(forLayering.ToArray(), blendModes.ToArray());
...
/*                var compositModes = Enum.GetValues(typeof(Enums.BlendMode))
                            .Cast<Enums.BlendMode>();
*/
                ...
                foreach(var mode in compositModes)
                {
                    ...
                    var filepathJoinedWithMeta = filepath + "-withmeta-joined.png";
                    framesWithMeta.Pngsave(filepathJoinedWithMeta);
                }
